	...
	if(BlockHolder==1) {
		...
		else if(BlockState==B)
		{
			...
			if((GPLocations[0].row > 0) && (GPLocations[0].col < TetrisCol)) {
				if(GPLocations[0].col == 0) {
					piecesFree += 1; // This is for piece 3 which takes piece 2's current position.
					if(!(GET_MATRIX_BIT(GPLocations[0].row - 1, GPLocations[0].col + 2))) {
						piecesFree += 1;
					}
		...
		else if(BlockState==D)
		{
			...
			//				0			
			if((GPLocations[0].row > 0) && (GPLocations[0].col < TetrisCol)) {
				if(GPLocations[0].col == TetrisCol - 1) {
					piecesFree += 1; // This is for piece 3 which takes piece 2's current position.
					if(!(GET_MATRIX_BIT(GPLocations[0].row + 1, GPLocations[0].col - 2))) {
						piecesFree += 1;
					}
		...
		{		
			...
			if((GPLocations[0].row > 1) && (GPLocations[0].col < TetrisCol)) {
				if(GPLocations[0].col == 0) {
					...
					if(!(GET_MATRIX_BIT(GPLocations[0].row - 2, GPLocations[0].col + 3))) {
						piecesFree += 1;
					}
				...
				else if(GPLocations[0].col == TetrisCol - 2) {
					checkPieceLeft = 1;
				...
				else if(GPLocations[0].col == TetrisCol - 1) {
					checkPieceLeft = 2;
					...
					if(!(GET_MATRIX_BIT(GPLocations[3].row - 1, GPLocations[3].col - 3))) {
						piecesFree += 1;
					}
					if((GET_MATRIX_BIT(GPLocations[2].row, GPLocations[2].col - checkPieceLeft))) {
						piecesFree = 0;
		...
		if(BlockState==A)
		{
			...
			if((GPLocations[0].row > 1) && (GPLocations[0].col < TetrisCol)) {
				if(GPLocations[0].col == TetrisCol - 2) {
		...
		else if(BlockState==D)
		{
			...
			if(GPLocations[2].col < TetrisCol) {
				if(GPLocations[2].col == TetrisCol - 1) {
					piecesFree += 1;
					if(!(GET_MATRIX_BIT(GPLocations[0].row + 2, GPLocations[0].col + 1))) {
						piecesFree += 1;
					}
		...
		else if(BlockState==B)
		{
			...
			if((GPLocations[1].row > 0) && (GPLocations[1].col < TetrisCol)) {
				if(GPLocations[0].col == 0) {
					...
					if(!(GET_MATRIX_BIT(GPLocations[3].row + 2, GPLocations[3].col + 1))) {
						piecesFree +=1;
					}
				}
				else {
		...
		if(!alreadyReOr && canOrient) {
			...
			if(BlockHolder==1)
			{
				...
				if(BlockState==A && canOrient)
				{
					...
					if(GPLocations[3].col == TetrisCol - 1) {
						GPLocations[0].row -= 2;
						GPLocations[0].col += 1;
						GPLocations[1].row -= 1;
						GPLocations[2].col -= 1;
